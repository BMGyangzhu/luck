...
public class VideoServiceImpl extends ServiceImpl<VideoMapper, Video> implements VideoService {
    ...
    public void publishVideo(Video video) {
        ...
        if (video.getLabelNames().isEmpty()) {
            String labelNames = type.getLabelNames();
        ...
        if (isAdd || !StringUtils.hasLength(oldVideo.getDuration())) {
            ...
            //在代码中，LocalCache 的使用主要是为了短期状态管理，防止视频时长的重复获取操作，
            // 同时清晰地标记操作的唯一性（通过 uuid）。
            // 它适合用在高并发和分布式场景下，既能提升性能，又能保持操作的幂等性(无论操作被执行多少次，其结果都与执行一次时的结果相同)。
    ...
    public Collection<Video> pushVideos(Long userId) {
        ...
        if (ObjectUtils.isEmpty(videoIds)) {
            // 如果 Redis 无数据（videoIds 为空），则回退到 SQL 查询最新视频
    ...
    // 将视频集合 videos 中每个视频的用户信息（UserVO）和视频格式（videoType）字段补全，避免N+1查询。
