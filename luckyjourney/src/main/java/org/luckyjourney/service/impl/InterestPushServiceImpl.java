...
//        redisCacheUtil.del(key);
        ...
        // TODO 为用户模型设置ttl，用户一段时间不上线之后清空用户模型
        ...
        if (user != null) {
            ...
            if (!ObjectUtils.isEmpty(modelMap)) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    // 标签对应的视频：是存在 redis 里面的。体育标签： 视频id1 视频id2 视频id3 
...
//                // 初始化布隆过滤器
//                BloomFilter<Long> bloomFilter = BloomFilter.create(Funnels.longFunnel(), 100_000, 0.01); // 10万条数据，误判率1%
//
//                // 使用布隆过滤器进行去重
//                Set<Long> filteredIds = ids.stream()
//                        .filter(id -> !bloomFilter.mightContain(id)) // 如果布隆过滤器中不存在该ID，则允许
//                        .peek(id -> bloomFilter.put(id)) // 如果过滤器中没有该元素，就将其加入
...
//                ids.addAll(filteredIds);
    ...
    public String[] initProbabilityArray(Map<Object, Object> modelMap) {
        ...
        // 初始化数组
        probabilityMap.forEach((labelsId, p) -> { // 遍历每个标签和它的概率数量
            int i = index.get();     // 获取当前的数组填充位置
            int limit = i + p;      // 计算当前标签填充的终止位置
            while (i < limit) {     // 从当前索引 i 开始填充到 limit
                probabilityArray[i++] = labelsId;
            }
            index.set(limit);       // 更新 index 的值为下次填充开始的位置
        });
