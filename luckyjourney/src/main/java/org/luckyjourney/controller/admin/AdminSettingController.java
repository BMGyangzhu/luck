...
public class AdminSettingController {
    ...
    public R update(@RequestBody @Validated Setting setting) throws JsonProcessingException {

        if (setting.getSettingScoreJson() != null) {
            String jsonStr = new ObjectMapper().writeValueAsString(setting.getSettingScoreJson());
            setting.setAuditPolicy(jsonStr); // 赋值给数据库字段
        }
